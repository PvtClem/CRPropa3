#include "crpropa/module/HadronicInteraction2.h"
...
HadronicInteraction::HadronicInteraction( ref_ptr<Density> gasDensity , bool haveElectrons, bool havePhotons, bool haveNeutrinos, double thinning, double limit) {
  setGasDensity(gasDensity);
...
void HadronicInteraction::setGasDensity( ref_ptr<Density> gasDensity ) {
...
std::vector<double> HadronicInteraction::setInteraction(Candidate *candidate) const { // given a particle, gets the primary total energies back, and uses it (and the cross-sections) to decide what secondary will be produced (provided an interaction occurs). Then we need to decide whether the interaction occurs or not (given the total cross-section computed here, in process), and then perform the interaction (in performInteraction)
  ...
  for (const auto& name : Filenames) { // get the files we are interested in
    if (name.find(chain) != std::string::npos && name.find("04L") == std::string::npos &&  name.find("cum") == std::string::npos) { //  the 04L are there only for cumulative tables, we don't want them now
        tab_filenames.push_back(name);
  ...
  double w = 1; // weigth of the particles
    ...
    candidate->addSecondary(22, E_sec / (1 + z) * GeV, pos, w, interactionTag); // photons # Em = 0
  	candidate->current.setEnergy((E - E_sec) / (1 + z) * GeV ); // E_sec is kinetic !! ; Em : rest energy
		...
		Vector3d pos = random.randomInterpolatedPosition(candidate->previous.getPosition(), candidate->current.getPosition());
    candidate->addSecondary(12, E_sec / (1 + z) * GeV, pos, w, interactionTag);
  	candidate->current.setEnergy((E - E_sec) / (1 + z) * GeV ); // E_sec is kinetic !! ; Em : rest energy
	}
    ...
    candidate->addSecondary(-12, E_sec / (1 + z) * GeV, pos, w, interactionTag);
  	candidate->current.setEnergy((E - E_sec) / (1 + z) * GeV ); // E_sec is kinetic !! ; Em : rest energy
		...
		Vector3d pos = random.randomInterpolatedPosition(candidate->previous.getPosition(), candidate->current.getPosition());
    candidate->addSecondary(14, E_sec / (1 + z) * GeV, pos, w, interactionTag);
  	candidate->current.setEnergy((E - E_sec) / (1 + z) * GeV ); // E_sec is kinetic !! ; Em : rest energy ; need to add the mass energy of each particle
	}
    ...
    candidate->addSecondary(-14, E_sec / (1 + z) * GeV, pos, w, interactionTag);
  	candidate->current.setEnergy((E - E_sec) / (1 + z) * GeV ); // E_sec is kinetic !! ; Em : rest energy
		...
		Vector3d pos = random.randomInterpolatedPosition(candidate->previous.getPosition(), candidate->current.getPosition());
    candidate->addSecondary(2112, (E_sec + mnc2) / (1 + z) * GeV, pos, w, interactionTag);
  	candidate->current.setEnergy((E - E_sec - mnc2) / (1 + z) * GeV ); // E_sec is kinetic !! ; Em : rest energy
		...
		Vector3d pos = random.randomInterpolatedPosition(candidate->previous.getPosition(), candidate->current.getPosition());
    candidate->addSecondary(-2112, (E_sec + mnc2) / (1 + z) * GeV, pos, w, interactionTag);
  	candidate->current.setEnergy((E - E_sec - mnc2) / (1 + z) * GeV ); // E_sec is kinetic !! ; Em : rest energy  // got all our particles for now
	}
...
void HadronicInteraction::process(Candidate *candidate) const {
  ...
  Vector3d pos = candidate->current.getPosition(); // to improve sligthly

  //std::cout <<  pos << std::endl;
  //std::cout << gasDensity->getHIDensity(pos) << "  " << gasDensity->getH2Density(pos)  << std::endl;

  std::vector<double> array = setInteraction(candidate); // return 2 values, CS_tot in 0, and secondary in 1

  double inverse_mfp = (gasDensity->getHIDensity(pos) + gasDensity->getH2Density(pos)) * array[0] * (barn * 1e-3)  ; //CS_tot computed before, put it in SI (from mb) // this one is not updated if we do several interactions in one step (the energy of the primary changes)
